...
where 
	...
	or
	orders_status is null ;

	
-- rename items column
alter table orders
rename column items to order_items ;
-- ----------------------------
-- Analysis & Reports
-- ----------------------------
...
-- filter for 1 last year and customer name 'Claire Ferguson'
-- group by customer_id dishes
...
-- Solving
-- create 2 columns for start and end time and GROUP by it and then count the orders
select 
...
-- 1 - calculate average amount for each customer group by customers and filter out those less than 25 orders
...
-- write query to find orders that placed but not delivered 
-- return each restaurant_name number of no delivered orders.
...
-- write query to caculate and compare the order cancellation rate for each restaurnt
...
-- segment customer into gold and silver groups based on thier total spending compared to the average order value 
-- if customers total spending exceed the aov label them as gold otherwise lable them as silver
select
	segmention ,
...
-- Find the number of 5_stars 4_satrs and 3_stars rating each rider has 
-- if orders delivered less than 15 min rider get 5_stars between 15 and 40 get 4 stars after 40 3 stars
select 
	riders_id , 
	stars ,
